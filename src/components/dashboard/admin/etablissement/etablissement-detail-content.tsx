...
export default function EtablissementDetailContent({ etablissement }: EtablissementDetailContentProps) {
  ...
  const sousDomaineRef = useRef<HTMLInputElement>(null)
  ...
  const handleUpdateEtablissement = async (e: FormEvent) => {
    e.preventDefault()
    setIsUpdating(true)
    setUpdateError(null)
    ...
    try {
      const updatedData = {
        ...
        sousDomaine: sousDomaineRef.current?.value,
        adresse: adresseRef.current?.value || null,
        codePostal: codePostalRef.current?.value || null,
        ville: villeRef.current?.value || null,
        telephone: telephoneRef.current?.value || null,
      ...
      const updatedEtablissement = await response.json()

      // Mettre à jour les données locales
      Object.assign(etablissement, updatedEtablissement)

      // Afficher le message de succès
      setUpdateSuccess("Les informations de l'établissement ont été mises à jour avec succès")

      // Rafraîchir la page pour afficher les données mises à jour
      router.refresh()
    } catch (error: any) {
      console.error("Erreur lors de la mise à jour:", error)
      setUpdateError(error.message || "Une erreur s'est produite lors de la mise à jour")
    } finally {
      setIsUpdating(false)
    }
  }
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <Card>
                    ...
                    <form onSubmit={handleUpdateEtablissement}>
                      <CardContent className="space-y-4">
                        {updateSuccess && (
                      ...
                      <CardFooter>
                        <Button
                          type="submit"
                          className="bg-[#c83e3e] hover:bg-[#b53535] flex items-center gap-2"
                          disabled={isUpdating}
                        >
                          {isUpdating ? (
                            <>
